...
// Same storage key as WorkflowsList
...
export const WorkflowTriggerPanel = () => {
  ...
  const loadWorkflows = () => {
    ...
    try {
      ...
      if (saved) {
        const allWorkflows = JSON.parse(saved) as Workflow[];
        // Only show active workflows
        setWorkflows(allWorkflows.filter(w => w.status === 'active'));
      } else {
        setWorkflows([]);
      }
    } catch (e) {
  ...
  useEffect(() => {
    ...
    const handleStorageChange = (e: StorageEvent) => {
      if (e.key === WORKFLOWS_STORAGE_KEY) {
        loadWorkflows();
      }
    };
          ...
          <CardTitle className="text-base flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-[10px] text-muted-foreground font-normal">Automations</p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <Button
        ...
        <CardContent className="flex-1 overflow-hidden pt-2 pb-3 flex flex-col">
          {isLoading ? (
            ...
            <div className="text-center py-6">
              ...
              <p className="text-xs text-muted-foreground mt-1">Create workflows in dashboard</p>
            </div>
          ) : (
              ...
              <div className="space-y-2 pr-2">
                {workflows.map((workflow) => {
                  ...
                  return (
                    <div
                      key={workflow.id}
                      className="p-2.5 rounded-lg border border-border bg-muted/30 hover:bg-muted/50 hover:border-purple-500/40 transition-colors"
                    >
                      <div className="flex items-center justify-between gap-2 mb-2">
                        <div className="min-w-0 flex-1">
                          <p className="font-medium text-sm truncate">{workflow.name}</p>
                          <Badge variant="outline" className={`gap-1 text-[10px] mt-1 ${triggerDisplay.color}`}>
                            <span>{typeof triggerDisplay.icon === 'string' ? triggerDisplay.icon : triggerDisplay.icon}</span>
