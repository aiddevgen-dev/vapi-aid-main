...
export const ChatPanel = ({ session, messages, onSendMessage }: ChatPanelProps) => {
  ...
  const handleSend = () => {
    console.log('🔥 ChatPanel: handleSend called with input:', input);
    if (!input.trim()) {
      console.log('⚠️ ChatPanel: Input is empty, not sending');
      return;
    }
    console.log('✅ ChatPanel: Calling onSendMessage with:', input.trim());
