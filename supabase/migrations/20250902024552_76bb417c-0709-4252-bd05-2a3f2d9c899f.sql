-- Create sample orders for Krishna Rao (user_id: c5820b77-a2de-4b81-af36-0d6ba7ef2a3c)

-- Order 1: Delivered order from 2 weeks ago
INSERT INTO orders (id, user_id, total_amount, status, shipping_address, created_at, updated_at) 
VALUES (
  'a1b2c3d4-e5f6-7890-abcd-ef1234567890',
  'c5820b77-a2de-4b81-af36-0d6ba7ef2a3c',
  234.98,
  'delivered',
  '{"name": "Krishna Rao", "street": "123 Oak Street", "city": "San Francisco", "postal_code": "94102", "country": "USA"}',
  now() - interval '14 days',
  now() - interval '12 days'
);

-- Order 2: Shipped order from 5 days ago
INSERT INTO orders (id, user_id, total_amount, status, shipping_address, created_at, updated_at)
VALUES (
  'b2c3d4e5-f6g7-8901-bcde-f23456789012',
  'c5820b77-a2de-4b81-af36-0d6ba7ef2a3c',
  154.98,
  'shipped',
  '{"name": "Krishna Rao", "street": "123 Oak Street", "city": "San Francisco", "postal_code": "94102", "country": "USA"}',
  now() - interval '5 days',
  now() - interval '3 days'
);

-- Order 3: Pending order from yesterday
INSERT INTO orders (id, user_id, total_amount, status, shipping_address, created_at, updated_at)
VALUES (
  'c3d4e5f6-g7h8-9012-cdef-345678901234',
  'c5820b77-a2de-4b81-af36-0d6ba7ef2a3c',
  89.99,
  'pending',
  '{"name": "Krishna Rao", "street": "123 Oak Street", "city": "San Francisco", "postal_code": "94102", "country": "USA"}',
  now() - interval '1 day',
  now() - interval '1 day'
);

-- Order Items for Order 1 (Air Jordan + Nike Hoodie)
INSERT INTO order_items (order_id, product_id, quantity, price, size, color)
VALUES 
  ('a1b2c3d4-e5f6-7890-abcd-ef1234567890', '56278a68-9f3f-4b05-bb98-d9e076e9ccaa', 1, 149.99, '10', 'Black/Red'),
  ('a1b2c3d4-e5f6-7890-abcd-ef1234567890', 'b9b1a6e4-2516-4c5b-8dfc-c32a5215bdee', 1, 89.99, 'L', 'Black');

-- Order Items for Order 2 (Adidas Ultraboost + Shorts)
INSERT INTO order_items (order_id, product_id, quantity, price, size, color)
VALUES 
  ('b2c3d4e5-f6g7-8901-bcde-f23456789012', '19477aec-b36a-47dd-827a-31172c5a9ce0', 1, 169.99, '9.5', 'White'),
  ('b2c3d4e5-f6g7-8901-bcde-f23456789012', 'b8c0859f-21a8-478c-98ae-6982dd37900d', 1, 34.99, 'M', 'Navy');

-- Order Items for Order 3 (Air Max 90)
INSERT INTO order_items (order_id, product_id, quantity, price, size, color)
VALUES 
  ('c3d4e5f6-g7h8-9012-cdef-345678901234', 'c78b0d4c-c283-474d-955f-acb7a3571848', 1, 89.99, '10.5', 'White/Grey');